...
def set_velocity_body(vx, vy, vz, yaw_rate=0.0):
...
def adjust_course_to_target(target_location):
    """
    Adjusts the drone's velocity to move towards the extended target with constant speed.
    """
    distance = get_distance_meters(target_location)
    
    # Maintain constant speed of 15 m/s
    vx = 15
...
def calculate_extended_target(current_location, target_location, distance_ahead):
    """
    Calculate a point ahead of the target location in the line between the current location and the target.
    """
    dlat = target_location.lat - current_location.lat
    dlon = target_location.lon - current_location.lon
    ...
    # Calculate new lat/lon by extending beyond the target location
    extended_lat = target_location.lat + dlat_norm * (distance_ahead / 1.113195e5)
    extended_lon = target_location.lon + dlon_norm * (distance_ahead / 1.113195e5)
    
    return LocationGlobalRelative(extended_lat, extended_lon, target_location.alt)
...
try:
    ...
    original_target = LocationGlobalRelative(-35.36181544, 149.16422522, 0)  # Example target
    
    current_location = vehicle.location.global_relative_frame
    extended_target = calculate_extended_target(current_location, original_target, 10)  # 10 meters ahead
    
    align_heading_to_target(extended_target)  # Align the UAV to face the extended target
    time.sleep(5)
    
    while True:
        distance_to_target = get_distance_meters(original_target)
        ...
        adjust_course_to_target(extended_target)
